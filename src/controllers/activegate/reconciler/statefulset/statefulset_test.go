...
func TestStatefulSet_TemplateSpec(t *testing.T) {
	...
	checkVolumeMounts := func(expected bool, templateSpec *corev1.PodSpec) {
		...
		assert.Equalf(t, expected, volumeIsDefined(templateSpec.Volumes, "auth-tokens"),
			"Expected that volume mount %s has a predefined pod volume", "auth-tokens",
		)
		assert.Equalf(t, expected, volumeIsDefined(templateSpec.Volumes, "ds-metadata"),
			"Expected that volume mount %s has a predefined pod volume", "ds-metadata",
		)
		assert.Equalf(t, expected, volumeIsDefined(templateSpec.Volumes, "extensions-logs"),
			"Expected that volume mount %s has a predefined pod volume", "extensions-logs",
		)
		assert.Equalf(t, expected, volumeIsDefined(templateSpec.Volumes, "statsd-logs"),
	...
	t.Run("DynaKube with StatsD enabled", func(t *testing.T) {
		...
		instance.Annotations[dynatracev1beta1.AnnotationFeatureEnableStatsDIngest] = "true"
...
func TestStatefulSet_Container(t *testing.T) {
	...
	assert.Equalf(t, instance.NeedsStatsD(), mountPathIsIn(activeGateContainer.VolumeMounts, activeGateConfigDir),
		"Expected that ActiveGate container defines mount point %s if and only if StatsD ingest is enabled", activeGateConfigDir,
	)
	assert.Equalf(t, instance.NeedsStatsD(), mountPathIsIn(activeGateContainer.VolumeMounts, extensionsLogsDir+"/eec"),
		"Expected that ActiveGate container defines mount point %s if and only if StatsD ingest is enabled", extensionsLogsDir+"/eec",
	)
	assert.Equalf(t, instance.NeedsStatsD(), mountPathIsIn(activeGateContainer.VolumeMounts, extensionsLogsDir+"/statsd"),
